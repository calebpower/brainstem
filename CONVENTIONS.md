...
| 5a metamorphic | Does a knob change only what it is allowed to? | Change one thing and require the rest of the conversation to be byte-identical: a frozen epoch may move the clock replies and nothing else, a seed may move its own echo and the random bytes and nothing else, a sorted walk may not depend on the order the entries were created in, and no reply may depend on what the working directory is called. Catches a value leaking from one field into another, which a pinned trace cannot see because it pins the leak too. |
