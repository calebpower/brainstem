...
| `--preopen-listen NAME=ADDR` | M7 | a bound, listening socket, ready to accept |
| `--preopen-connect NAME=ADDR` | M7 | an already-connected socket |
...
#
#   hello  op 01, len 10.  No preopens here: this program touches no
#   filesystem at all, which is also the point -- see ABI.md section 8.1.
EMIT 01 0a 00 42 53 54 4d 01 00 00 00 00 00
READ 51
#   socket  op 05, domain 1 IPv4, type 1 stream, protocol 0.  Handle 1.
...
EMIT 07 26 00 01 00 00 00 01 00
...
READ 28
#   listen  op 08, handle 1, backlog 0 meaning 128
EMIT 08 06 00 01 00 00 00 00 00
READ 3
#   socket  op 05 again.  Handle 2, the client.
EMIT 05 03 00 01 01 00
READ 7
#   connect  op 06, handle 2, flags 0, family 01, reserved 00 ...
EMIT 06 26 00 02 00 00 00 00 00 01 00
...
EMIT 00 00 00 00 00 00 00 00 00 00 00 00
READ 3
#   accept  op 09, handle 1, flags 0.  The reply is a handle and the peer
#   address: 3 header + 4 + 32 = 39 bytes.  Handle 3.
EMIT 09 06 00 01 00 00 00 00 00
READ 39
#   write  op 0b, handle 2 (the client end), flags 0, "hi"
EMIT 0b 08 00 02 00 00 00 00 00 68 69
READ 5
#   read  op 0a, handle 3 (the accepted end), n 8, flags 0.  Two bytes come
#   back, and they came through a real TCP socket.
EMIT 0a 08 00 03 00 00 00 08 00 00 00
READ 5
